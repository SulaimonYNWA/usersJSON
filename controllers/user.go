...
func (h UserController) AddUser(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		re.Error(err.Error(), http.StatusBadRequest, w, false)
		return
	}
	...
	if err != nil {
		re.Error("json unmarshall: "+err.Error(), http.StatusBadRequest, w, true)
		return
	}
	...
	if err != nil {
		...
		re.Error(err.Error(), http.StatusInternalServerError, w, true)
		return
	}

	re.Reply(id, http.StatusCreated, w, false)
}
...
func (h UserController) GetAll(w http.ResponseWriter, req *http.Request) {
	...
	re.Reply(users, http.StatusOK, w, false)
}
...
func (h UserController) GetByID(w http.ResponseWriter, req *http.Request) {
	...
	if err == sql.ErrNoRows { // user by id not exist
		re.Reply(http.StatusText(http.StatusNotFound), http.StatusNotFound, w, false)
		return
	}
	if err != nil {
	...
	re.Reply(*user, http.StatusOK, w, false)
}
